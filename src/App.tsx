...
// Create a draggable component for product items
const DraggableProductItem = ({
  ...
  return (
    <div
      ...
      {...attributes}
    >
      <div className="h-14 w-14 bg-gray-100 rounded border border-gray-200 flex-shrink-0 overflow-hidden cursor-grab">
        <img
          src={product.image}
          alt={product.name}
...
// Design Canvas component with droppable area
const DesignCanvas = ({
  children,
  mainImage,
  emptyStateMessage,
  ...
  // Add a subtle highlight when a draggable is over the drop zone
  ...
  return (
    <div
      ref={setNodeRef}
      className={`relative bg-white rounded shadow-lg overflow-hidden !cursor-default ${borderStyle}`}
    ...
    >
      ...
      <div className="w-full h-full flex items-center justify-center relative">
        {mainImage ? children : emptyStateMessage}
      </div>
    </div>
  );
};
...
function App() {
  ...
  const mouseSensor = useSensor(MouseSensor, {
    // Make it easier to start dragging
  ...
  const touchSensor = useSensor(TouchSensor, {
    // Make it easier to start dragging on touch devices
    activationConstraint: {
      distance: 5, // Start dragging after moving 5px
    },
  });
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    // Only process if we have a drop target with id 'design-canvas'
    if (event.over && event.over.id === "design-canvas") {
      const productId = event.active.id as string;
      console.log("Product dropped:", productId);

      // Select the product - this will update mainImage and selectedProduct
      handleSelectProduct(productId);
    }

    // Set activeId to null after a short delay to prevent the sudden "jump back" animation
    setTimeout(() => {
      setActiveId(null);
    }, 10);
  };
  ...
  const renderDragOverlay = () => {
    ...
    return (
      <div className="h-32 w-32 bg-white rounded-lg border-2 border-blue-400 overflow-hidden shadow-xl z-50">
        <img
          src={product.image}
          alt={product.name}
          className="h-full w-full object-contain"
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center text-gray-400">
      ...
      <p className="text-sm">
        Select a product from the list or drag and drop a product here
      </p>
    </div>
  );
  ...
  return (
    ...
    >
          ...
          <div className="w-64 bg-white border-r border-gray-200 overflow-auto">
            <div className="p-4">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-medium text-gray-800">Products</h2>
                <button
                  onClick={handleReset}
                  className="text-xs text-blue-600 hover:text-blue-800 bg-blue-50 px-2 py-1 rounded font-semibold"
                >
                  Reset Canvas
                </button>
              </div>
            ...
            <div className="bg-white border-b border-gray-200 px-4 py-2 flex justify-between items-center">
              ...
              <div className="flex items-center space-x-2">
                {selectedProduct && (
                  <button
                    onClick={handleReset}
                    className="bg-gray-200 text-gray-700 px-3 py-1.5 text-sm rounded-md hover:bg-gray-300 mr-2"
                  >
                    Clear Canvas
                  </button>
                )}
                    ...
                    >
                      ...
                      <div className="absolute bottom-10 left-8 right-8 bg-white p-4 rounded-lg shadow-md border border-gray-200">
                        ...
                        <p className="text-sm text-gray-600">
                          ...
                          performance. Perfect for everyday use, it will quickly
            ...
            <div className="h-8 bg-white border-t border-gray-200 flex items-center justify-between px-4">
              ...
              <div className="flex items-center text-xs text-gray-500">
                <span>
                  ...
                  <strong>Drag products</strong> to canvas
                </span>
              </div>
            </div>
          </div>
      ...
      <DragOverlay zIndex={1000}>{renderDragOverlay()}</DragOverlay>
    </DndContext>
  );
}
